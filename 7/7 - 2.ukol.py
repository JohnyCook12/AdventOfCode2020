...
with open("input.txt", "r") as myfile:
    ...
    """ ######## 2 úkol ######## """
    # kolik tašek je uvnitř shiny_gold?
    print(bag_list)
    print(obsah_tasky('shiny gold'))

    def pocet_tasek_primo_uvnitr(taska: str) -> int:
        finalni_soucet = 0
        for polozka in obsah_tasky(taska):
            finalni_soucet += int(polozka[0])
        return finalni_soucet




    def pocet_tasek_neprimo_uvnitr(taska: str) -> int:
        final_count = 0
        for polozka in obsah_tasky(taska):
            if polozka == 'no other bags.':
                break
            else:
                final_count += (int(polozka[0])+int(polozka[0])*pocet_tasek_neprimo_uvnitr(polozka[2:]))
        return final_count

    print(pocet_tasek_neprimo_uvnitr('shiny gold'))